...
### FC(フライトコントローラー)
受信機から送信されたデータ、センサーによって取得したデータ等をもとに、モーターを制御する。
...
以上の理由から、マルチコプターでは機体全体のモーメントが釣り合うように、対角線上のプロペラの回転方向を一致させ、隣り合うプロペラの回転方向を逆向きにする。
...
### 前後(x, y軸)
移動したい方向のモーターの回転数(rpm)を相対的に下げることで移動を行う。

![](../images/fig3.drawio.png){ width=30% }
![](../images/fig6.drawio.png){ width=30% }

### yaw 回転　(z軸周りの回転)
機体を、yaw軸周りに回転させることを考える。入力(回転したい向き)に対して、逆向きに回転しているモーターの回転数を大きくすることで、機体は入力した向きに回転する。

この挙動は機体全体で、反トルクの合計が回転したい向きになるために生じる。
